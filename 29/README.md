...
## 1. カテゴリーの書き方がマルチカラムアトリビュートになりそう。

### 現状

注文表から、以下のようなカラムと値を考えていた。
...
しかしこのような場合、さらに階層が深くなった場合にいわゆるマルチカラムアトリビュートのようになってしまうのではないかとアドバイスをもらった。

### 修正案

カテゴリーとタグという考えに分けて考える。
...
![refactor-pair-review-1](./db/refactor-pair-review-1.png)

ついでに、シャリサイズやサビ抜きステータスを正規化した。
（サーティーワンフレーバー対策
...
- マルチカラムアトリビュートにならない。
- タグとカテゴリーという 2 つの性質を分けて考えることができた
- 1 つの商品に複数のタグを付与することが可能になった

## 2. キャンペーン施策対応

### 現状

キャンペーン施策への対応として、伝票テーブルに合計金額を追加した。
なぜなら、`商品金額*個数 != 合計金額` であり、導出不可能である。つまり合計金額の意味が異なるため、冗長性の排除が不可能であるから。

対策としては良さそうなので、nakano-san から提案してくれたキャンペーンコードに関して取り組む
...
| キャンペーン ID | キャンペーン名                                                           | 開始日時            | 終了日時            |
| --------------- | ------------------------------------------------------------------------ | ------------------- | ------------------- |
| 1               | 一皿 220 円(税別)以上のお好みすしで同じネタを 2 つおきの購入で 40 円割引 | 2021-01-01 00:00:00 | 2021-03-31 23:59:59 |
| 2               | 一皿 100 円(税別)のお好みすしを 80 円で提供                              | 2021-04-01 00:00:00 | 2021-04-13 23:59:59 |

![refactor-pair-review-2](./db/refactor-pair-review-2.png)

### 改善点

- キャンペーンテーブルを作成した
- 中間テーブルを作成した
- DB でキャンペーンに関する制御は実施せずに、計算ロジックはアプリケーション側で対応する
